...
export function add(repoPath: string, filePath: string): void {
...
export function createTree(repoPath: string): string {
  const indexPath = path.join(repoPath, GIT_DIR, "index");
...
export function commit(repoPath: string, message: string): void {
  ...
  const author =
    "Arturs <test.noreply.github.com>";
  ...
  commitContent += `committer ${author} ${Math.floor(
    Date.now() / 1000
  )} +0000\n\n`;
...
export function status(repoPath: string): void {
  const indexPath = path.join(repoPath, GIT_DIR, "index");
  const indexEntries = fs.existsSync(indexPath)
    ? fs.readFileSync(indexPath, "utf-8").split("\n").filter(Boolean)
    : [];
  ...
  const trackedFiles = new Set(
    indexEntries.map((entry) => entry.split(" ")[1])
  );
  const currentFiles = new Set(
    fs.readdirSync(repoPath).filter((file) => file !== GIT_DIR)
  );
...
export function setRemote(repoPath: string, remoteName: any, remoteUrl: any) {
...
export function push(repoPath: string, remoteName: string, branch: string, username: string, token: string) {
    try {
